...
def local_search(pop, toolbox, stats=None,
    ...
    print("fitness values: ", pop[0].fitness.values)
    ...
    while not all(finished) and gen <= config.GEN_NUM:

        print("finished: ", sum(finished))
        ...
        if new_fit > current_fit:
            ...
            finished[current_idx] = False
        else:
            finished[current_idx] = True
        current_idx += 1
        current_idx %= num_of_nodes
...
def get_neighbours(nodes):
    ...
    # delete existing node at relevant position
    del nodes[0]
